...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($fn_err) && empty($em_err) && empty($pwd_err)) {
        ...
        $query = "INSERT INTO furniture (id, firstname, email, password) VALUES ('$id', '$firstname', '$email', '$hashed_password')";
        if (mysqli_query($con, $query)) {
            $success_msg = "Registration successful! Redirecting to login...";
            header("refresh:2;url=login.php");
        } else {
            $success_msg = "Error: " . mysqli_error($con); // Debugging message
        }
    }
}
?>
...
<div class="registration-form">
    <form action="" method="POST">
        ...
        <input type="password" class="box" name="ps" placeholder="Enter Password" autocomplete="off">
